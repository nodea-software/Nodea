const attributes = require("./attributes/e_chat.json");
const options = require("./options/e_chat.json");
const modelBuilder = require('@core/helpers/model_builder');
const defaultHooks = require('@core/models/hooks');

const modelName = 'E_chat';
const tableName = 'e_chat_chat';

const { DataTypes, Model } = require('sequelize');

class E_chat extends Model {
	...
	static load(sequelize) {
		...
		for (const hookType in hooks)
			for (const hook of hooks[hookType])
				E_chat.addHook(hookType, hook.name, hook.func)
	}
