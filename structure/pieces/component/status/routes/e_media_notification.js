...
const options = require('@app/models/options/e_media_notification');
const attributes = require('@app/models/attributes/e_media_notification');
...
class MediaNotification extends Entity {
	constructor() {
		const additionalRoutes = [];
		super('e_media_notification', attributes, options, helpers, additionalRoutes);
	}
	...
	get hooks() {
		return {
			...
			create: {
				...
				beforeRedirect: async(data) => {
					...
					const media = await models.E_media.create({
						...
						fk_id_media_notification: createdRow.id
			...
			update: {
				start: async (data) => {
					data.idEntity = data.req.body.id_media_notification;
				},
				beforeRedirect: async(data) => {
					const {req, res, updateRow, transaction} = data;
					const mediaNotification = updateRow;
					const e_media = await models.E_media.findOne({where: {fk_id_media_notification: mediaNotification.id}});
					if (!e_media)
				...
				beforeDestroy: async(data) => {
					...
					try {
						await models.E_media.destroy({
							where: {fk_id_media_notification: deleteId},
							transaction: data.transaction
						});
					} catch(err) {
						console.error("MediaNotification: Couldn't delete associated Media - id="+deleteId);
						console.error(err);
					}
				},
				// beforeRedirect: async(data) => {},
