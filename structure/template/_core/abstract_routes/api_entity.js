...
class ApiEntity extends ApiRoute {
	constructor(e_entity, attributes, options, additionalRoutes) {
		...
		this.defaultMiddlewares.push(middlewares.apiAuthentication, middlewares.apiEntityAccess(this.entity), matomoTracker);
	}
	...
	find() {
		this.router.get(
			...
			this.asyncRoute(async (data) => {
				...
				if ((await this.getHook("find", "beforeFind", data)) === false) return;
				...
				if ((await this.getHook("find", "afterFind", data)) === false) return;
	...
	findOne() {
		this.router.get(
			...
			this.asyncRoute(async (data) => {
				...
				if ((await this.getHook("findOne", "beforeFind", data)) === false) return;

				const result = await models[this.E_entity].findOne(data.query);
				if (!result) return data.res.error((_) => data.res.status(404).json({ error: "No " + this.entity + " with ID " + data.id }));
				data.answer[this.entity] = result;

				if ((await this.getHook("findOne", "afterFind", data)) === false) return;
	...
	findAssociation() {
		this.router.get(
			"/:id/:association",
			...this.middlewares.findAssociation,
			this.asyncRoute(async (data) => {
				...
				if ((await this.getHook("findAssociation", "beforeFind", data)) === false) return;

				const result = await models[this.E_entity].findOne(data.query);
				if (!result) return data.res.error((_) => data.res.status(404).json({ error: "No " + this.entity + " with ID " + data.id }));
				data.answer[data.association] = result[data.query.include.as];

				if ((await this.getHook("findAssociation", "afterFind", data)) === false) return;
	...
	create() {
		this.router.post(
			...
			this.asyncRoute(async (data) => {
				...
				if ((await this.getHook("create", "beforeCreate", data)) === false) return;
				...
				if ((await this.getHook("create", "beforeAssociations", data)) === false) return;

				// Set associations
				await Promise.all(data.createAssociations.map((asso) => result[asso.func](asso.value, { transaction: data.transaction })));

				if ((await this.getHook("create", "afterAssociations", data)) === false) return;
	...
	update() {
		this.router.put(
			...
			this.asyncRoute(async (data) => {
				...
				if ((await this.getHook("update", "beforeAssociations", data)) === false) return;
				...
				if ((await this.getHook("update", "afterAssociations", data)) === false) return;
