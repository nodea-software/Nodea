	...
	$.fn.gridEditor = function(options) {
		...
		self.each(function(baseIndex, baseElem) {
			...
			function setup() {
				...
				// Buttons on right
				var layoutDropdown = $('<div class="dropdown float-right ge-layout-mode">' +
						'<button type="button" class="btn btn-xs btn-primary dropdown-toggle" data-toggle="dropdown"><span>Desktop</span></button>' +
						'<ul style="cursor: pointer;" aria-labelledby="build-dropdown" class="dropdown-menu border-0 shadow">' +
						'<li><a class="dropdown-item" title="Desktop" data-width="auto"><span>Desktop</span></a></li>' +
						'<li><a class="dropdown-item" title="Tablet"><span>Tablet</span></li>' +
						'<li><a class="dropdown-item" title="Phone" id="custom-grid-editor-print-layout"><span>Phone</span></a></li>' +
						'</ul>' +
						'</div>')
					.on('click', 'a', function() {
						var a = $(this);
			...
			function createRowControls() {
				canvas.find('.row').each(function() {
					...
					var drawer = $('<div class="ge-tools-drawer" style="width:101%" />').prependTo(row);
			...
			function createColumn(size) {
				...
				return $('<div style="min-height: 75px;" />').addClass("emptySpaceColumn").addClass(colClasses.map(function(c) {
					return c + size;
				}).join(' '));
			}
